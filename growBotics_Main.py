...
# Modified by Rob Kramer for growBonics - 2016
#
...
def create_relay_tables():
    ...
    for tablename in relaytimer:
        curs.execute("CREATE TABLE IF NOT EXISTS {} "
                    ...
                    "cycles_on INTEGER DEFAULT NULL, "
		    "cycles_off INTEGER DEFAULT NULL);".format(tablename))

    # Add default "NULL" data to each relay_timer table

        for pairs in range(1, (numdtpairs[dtcount] + 1)):
            curs.execute("INSERT IGNORE INTO {} (pk,starttime,stoptime,cycles_on,cycles_off)"
                        " VALUES({},NULL,NULL,NULL,NULL)".format(tablename, pairs))
        dtcount += 1
...
def read_sensors():
    ...
    # Get the readings from any Atlas Scientific temperature sensors
            ...
            if value["sensor_type"] == "atlas_scientific_temp":
                ...
                sensor_reading = round(float(device.query("R")*(9.0/5.0) + 32),
                                value["accuracy"])
                all_curr_readings.append([value["name"], sensor_reading])
                if value["test_for_alert"] is True:
                    alert_readings.append([value["name"], sensor_reading])
...
def send_email(alert_readings):
    ...
    fromaddr = "unsecureGrowBotics@gmail.com"
    toaddr = all_settings["to_email"]
    alladdr = toaddr.split(",")
    msg = MIMEMultipart()
    msg['From'] = fromaddr
    msg['To'] = toaddr
    msg['Subject'] = "growBotics Alert"
...
def check_each_start_stop_timer(timer_data, rct):

    # If the relay is set to auto then check all start/stop times for that
    # relay and based on current time turn relay on or off

    global cycleCounters
    global states
    cdt = datetime.datetime.now()
    if timer_data[1] is None or timer_data[3] is None:  # Python reads a "NULL" value as None
        return  False
        ...
        if (timer_data[0] == profile): 
            if states[rct]: 
                if cycleCounters[rct] >= timer_data[3]:
		    states[rct] = not states[rct]
		    cycleCounters[rct] = 0
		else:
		    cycleCounters[rct] += 1
	    else:
		if cycleCounters[rct] >= timer_data[4]:
		    states[rct] = not states[rct]
		    cycleCounters[rct] = 0
		else:
	    	    cycleCounters[rct] += 1
    ...
    for rct, opp, ndtp, rts in zip(relaycount, outputpins,
        ...
        if override[rct] == "auto":
            ...
            while relayon is False and dtpair <= ndtp:
                ...
                relayon = check_each_start_stop_timer(timer_data, rct)
                if relayon is True:
...
# NOTE: When modifying this table, delete any old readings in sensors table
...
sensors = OrderedDict([("temp_1", {  # DS18B20 Temperature Sensor
                            ...
                            "is_ref": True,
                            "ds18b20_file":
                            "/sys/bus/w1/devices/28-031561c7b3ff/w1_slave",
                            "accuracy": 1,
                            "test_for_alert": True,
                       ...
                       ("atlas_sensor_4", {  # pH/ORP Atlas Scientific Sensor
                            ...
                            "upper_alert_value": 6.3,
                            "lower_alert_name": "ph_low",
                            "lower_alert_value": 5.8})])
...
numdtpairs = [2, 2, 2, 2]  # Number of Start/Stop pairs for each relay
relaycount = range(1, (len(outputpins) + 1))
cycleCounters = [cycleCounter0,cycleCounter1,cycleCounter2,cycleCounter3,cycleCounter4]
...
loops = time_between_readings

#remove_excess_timer_override_and_relay_database_entries()
#remove_excess_datetime_pairs()
remove_unused_sensors()

#initialize relay loop counters to all relays start on ON cycle

for rct, opp, ndtp, rts in zip(relaycount, outputpins,
                               numdtpairs, relay_timer_names):
    timer_data = get_relay_timer_start_stop_data(rts, profile)
    cycleCounters[rct] = timer_data[4]
